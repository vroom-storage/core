#!/bin/bash
set -e

scriptdir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cluster_url="${1:-http://localhost:8080}"
test_bucket="hadoop-s3a-test"
image_name="ghcr.io/stjank/hadoop-s3a-tests:v2"

export VRM_ROOT_USER="main"
export VRM_ROOT_ACCESS_KEY="0555b35654ad1656d804"
export VRM_ROOT_SECRET_KEY="h7GhxuBLTrlhVUyxSPUKUV8r/2EI4ngqJxD7iBdBYLhwluN30JaT3Q=="

export AWS_ACCESS_KEY_ID="$VRM_ROOT_ACCESS_KEY"
export AWS_SECRET_ACCESS_KEY="$VRM_ROOT_SECRET_KEY"
export AWS_DEFAULT_REGION="us-east-1"
aws --endpoint-url "$cluster_url" --no-verify-ssl s3 mb "s3://$test_bucket"

docker_args=(--rm --network host
    -e "S3_ENDPOINT=$cluster_url"
    -e "S3_ACCESS_KEY=$VRM_ROOT_ACCESS_KEY"
    -e "S3_SECRET_KEY=$VRM_ROOT_SECRET_KEY"
    -e "S3_BUCKET=$test_bucket"
)
skip_file="$scriptdir/hadoop_s3a_skip.txt"
[ -f "$skip_file" ] && docker_args+=(-v "$skip_file:/hadoop_s3a_skip.txt:ro")

[ -n "${IT_TEST:-}" ] && docker_args+=(-e "IT_TEST=$IT_TEST")

exec docker run "${docker_args[@]}" "$image_name"
